...
namespace Kontenu.Master {
    public partial class FrmTujuanProyekAdd : DevExpress.XtraEditors.XtraForm {
        private String id = "TUJUANPROYEK";
        private String dokumen = "TUJUANPROYEK";
        private Boolean isAdd;

        public FrmTujuanProyekAdd(bool pIsAdd) {
            isAdd = pIsAdd;
            InitializeComponent();
        }

        private void FrmTujuanProyekAdd_Load(object sender, EventArgs e) {
        ...
        private void setDefaultInput(MySqlCommand command) {
            if(!this.isAdd) {
                ...
                txtNama.Text = dTujuanProyek.nama;
            } else {
        ...
        private void btnSimpan_Click(object sender, EventArgs e) {
            ...
            try {
                ...
                if(this.isAdd) {
                    dTujuanProyek.tambah();
                    // update kode header --> setelah generate
                    strngKode = dTujuanProyek.kode;
                } else {
                    dTujuanProyek.ubah();
                }

                // tulis log
                OswLog.setTransaksi(command, dokumen, dTujuanProyek.ToString());

                // reload grid di form header
                FrmTujuanProyek frmTujuanProyek = (FrmTujuanProyek)this.Owner;
                frmTujuanProyek.setGrid(command);
