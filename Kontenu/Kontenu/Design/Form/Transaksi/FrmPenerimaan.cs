    ...
    public partial class FrmPenerimaan : DevExpress.XtraEditors.XtraForm
    {
        ...
        public void setGrid(MySqlCommand command)
        {
            ...
            String query = @"SELECT A.kode AS Nomor, A.tanggal AS Tanggal, B.nama AS 'Nama Klien', C.nama AS 'Nama Proyek', AA.kode AS Invoice, 
                            ...
                            INNER JOIN invoice AA ON AAA.invoice = AA.kode
                            ...
                            WHERE toDate(A.tanggal) BETWEEN toDate(@tanggalawal) AND toDate(@tanggalakhir) AND 
	                            A.kode LIKE @kode AND B.nama LIKE @klien AND AA.kode LIKE @invoice
                            ORDER BY A.kode";
            ...
            gridControl = OswGrid.getGridBrowse(gridControl, command, query, parameters,
                                                new String[] { },
                                                new String[] { "Grand Total", "Diterima" });
        }
        ...
        {
            FrmPenerimaanAdd form = new FrmPenerimaanAdd(false);
            MySqlConnection con = new MySqlConnection(OswConfig.KONEKSI);
            MySqlCommand command = con.CreateCommand();
            MySqlTransaction trans;
        ...
        {
            FrmPenerimaanAdd form = new FrmPenerimaanAdd(true);
            this.AddOwnedForm(form);
            form.ShowDialog();
        }
