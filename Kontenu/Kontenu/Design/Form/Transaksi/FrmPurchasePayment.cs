...
{
    public partial class FrmPurchasePayment : DevExpress.XtraEditors.XtraForm
    {
        ...
        public void setGrid(MySqlCommand command)
        {
            ...
            String query = @"SELECT A.kode AS Nomor, A.tanggal AS Tanggal, B.nama AS Outsource, 
                            ...
                            WHERE toDate(A.tanggal) BETWEEN toDate(@tanggalawal) AND toDate(@tanggalakhir) AND 
                                  A.kode LIKE @kode AND B.kode LIKE @outsource
                            ORDER BY A.kode";
        ...
        {
            FrmPurchasePaymentAdd form = new FrmPurchasePaymentAdd(false);
            MySqlConnection con = new MySqlConnection(OswConfig.KONEKSI);
            MySqlCommand command = con.CreateCommand();
            MySqlTransaction trans;
        ...
        {
            FrmPurchasePaymentAdd form = new FrmPurchasePaymentAdd(true);
            this.AddOwnedForm(form);
            form.ShowDialog();
        }
        ...
        private void btnHapus_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                if (OswPesan.pesanKonfirmasiHapus() == DialogResult.Yes)
                {
                    ...
                    dPurchasePayment.hapus();

                    // tulis log
                    OswLog.setTransaksi(command, "Hapus " + dokumen, dPurchasePayment.ToString());
