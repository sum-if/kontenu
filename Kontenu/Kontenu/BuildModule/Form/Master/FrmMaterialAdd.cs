...
namespace Kontenu.Master {
    public partial class FrmMaterialAdd : DevExpress.XtraEditors.XtraForm {
        ...
        private String dokumen = "MATERIAL";
        private Boolean isAdd;

        public FrmMaterialAdd(bool pIsAdd) {
            isAdd = pIsAdd;
            InitializeComponent();
        }
        ...
        private void FrmMaterialAdd_Load(object sender, EventArgs e) {
            ...
            try {
                ...
                cmbKategoriMaterial = ComboQueryUmum.getKategoriMaterial(cmbKategoriMaterial, command);
        ...
        private void setDefaultInput(MySqlCommand command) {
            if(!this.isAdd) {
                ...
                DataMaterial dMaterial = new DataMaterial(command, strngKode);
                txtNama.Text = dMaterial.nama;
                cmbKategoriMaterial.EditValue = dMaterial.kategorimaterial;
                cmbUnit.EditValue = dMaterial.unit;
            } else {
                OswControlDefaultProperties.resetAllInput(this);

                cmbKategoriMaterial.ItemIndex = 0;
                cmbUnit.ItemIndex = 0;
            }
        ...
        private void btnSimpan_Click(object sender, EventArgs e) {
            ...
            try {
                ...
                dxValidationProvider1.SetValidationRule(cmbKategoriMaterial, OswValidation.IsNotBlank());
                ...
                String strngKategoriMaterial = cmbKategoriMaterial.EditValue.ToString();
                String strngUnit = cmbUnit.EditValue.ToString();

                DataMaterial dMaterial = new DataMaterial(command, strngKode);
                dMaterial.nama = strngNama;
                dMaterial.kategorimaterial = strngKategoriMaterial;
                dMaterial.unit = strngUnit;

                if(this.isAdd) {
                    dMaterial.tambah();
                    // update kode header --> setelah generate
                    strngKode = dMaterial.kode;
                } else {
                    dMaterial.ubah();
                }

                // tulis log
                // OswLog.setTransaksi(command, dokumen, dMaterial.ToString());

                // reload grid di form header
                FrmMaterial frmMaterial = (FrmMaterial)this.Owner;
                frmMaterial.setGrid(command);
