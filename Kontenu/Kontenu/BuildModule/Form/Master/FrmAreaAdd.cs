...
namespace Kontenu.Master {
    public partial class FrmAreaAdd : DevExpress.XtraEditors.XtraForm {
        private String id = "AREA";
        private String dokumen = "AREA";
        private Boolean isAdd;

        public FrmAreaAdd(bool pIsAdd) {
            isAdd = pIsAdd;
            InitializeComponent();
        }

        private void FrmAreaAdd_Load(object sender, EventArgs e) {
        ...
        private void setDefaultInput(MySqlCommand command) {
            if(!this.isAdd) {
                ...
                DataArea dArea = new DataArea(command, strngKode);
                txtNama.Text = dArea.nama;
            } else {
        ...
        private void btnSimpan_Click(object sender, EventArgs e) {
            ...
            try {
                ...
                DataArea dArea = new DataArea(command, strngKode);
                //kode diinput manual
                dArea.kode = strngKode;
                dArea.nama = strngNama;

                if(this.isAdd) {
                    dArea.tambah();
                    // update kode header --> setelah generate
                    strngKode = dArea.kode;
                } else {
                    dArea.ubah();
                }

                // tulis log
                // OswLog.setTransaksi(command, dokumen, dArea.ToString());

                // reload grid di form header
                FrmArea frmArea = (FrmArea)this.Owner;
                frmArea.setGrid(command);
