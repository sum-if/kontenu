...
namespace Kontenu.Akuntansi {
    class DataKasKeluar {
        ...
        public DataKasKeluar(MySqlCommand command, String kode) {
        ...
        public void tambah() {
            ...
            String query = @"INSERT INTO kaskeluar(kode, tanggal, catatan, akun, total,nourut,version,create_user) 
        ...
        public void hapus() {
            ...
            // hapus header
            String query = @"DELETE FROM kaskeluar
                             WHERE kode = @kode";
        ...
        public void hapusDetail() {
            ...
            foreach(DataRow row in dataTable.Rows) {
                DataKasKeluarDetail dKasKeluarDetail = new DataKasKeluarDetail(command, this.kode, row["no"].ToString());
                dKasKeluarDetail.hapus();
            }
        ...
        private void valVersion() {
            DataKasKeluar dKasKeluar = new DataKasKeluar(command, this.kode);
            if(this.version != dKasKeluar.version) {
        ...
        public void valJumlahDetail() {
            String query = @"SELECT COUNT(*)
                             FROM kaskeluardetail A
                             WHERE A.kaskeluar = @kaskeluar";
        ...
        public void prosesJurnal() {
            ...
            DataJurnal dJurnal = new DataJurnal(command, this.id, this.kode, this.tanggal, strngKeteranganHeader, (no++).ToString(), this.akun, "0", this.total);
            ...
            foreach(DataRow row in dataTable.Rows) {
                ...
                dJurnal = new DataJurnal(command, this.id, this.kode, this.tanggal, strngKeteranganDetail, (no++).ToString(), strngAkun, strngJumlah, "0");
                dJurnal.prosesJurnal();
            }
