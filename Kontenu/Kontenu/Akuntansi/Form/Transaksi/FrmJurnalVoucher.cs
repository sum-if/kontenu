...
namespace Kontenu.Akuntansi {
    public partial class FrmJurnalVoucher : DevExpress.XtraEditors.XtraForm {
        private String id = "JURNALVOUCHER";
        private String dokumen = "JURNALVOUCHER";

        public FrmJurnalVoucher() {
            InitializeComponent();
        }

        private void FrmJurnalVoucher_Load(object sender, EventArgs e) {
        ...
        public void setGrid(MySqlCommand command) {
            ...
            String query = @"SELECT A.kode AS Nomor, A.tanggal AS Tanggal, A.catatan AS Catatan
                            FROM jurnalvoucher A
                            WHERE toDate(A.tanggal) BETWEEN toDate(@tanggalawal) AND toDate(@tanggalakhir) AND                                   
                                  A.kode LIKE @kode AND A.catatan LIKE @catatan
                            ORDER BY A.kode";
        ...
        private void btnUbah_Click(object sender, EventArgs e) {
            FrmJurnalVoucherAdd form = new FrmJurnalVoucherAdd(false);
            MySqlConnection con = new MySqlConnection(OswConfig.KONEKSI);
            MySqlCommand command = con.CreateCommand();
            MySqlTransaction trans;
        ...
        private void btnTambah_Click(object sender, EventArgs e) {
            FrmJurnalVoucherAdd form = new FrmJurnalVoucherAdd(true);
            this.AddOwnedForm(form);
            form.ShowDialog();
        }
