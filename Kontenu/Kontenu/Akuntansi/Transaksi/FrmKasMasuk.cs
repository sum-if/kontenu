...
namespace Kontenu.Akuntansi {
    public partial class FrmKasMasuk : DevExpress.XtraEditors.XtraForm {
        ...
        private void FrmKasMasuk_Load(object sender, EventArgs e) {
            ...
            try {
                ...
                cmbKasMasuk = ComboQueryUmum.getAkun(cmbKasMasuk, command, "akun_input_kas_masuk", true);
                cmbKasMasuk.EditValue = OswCombo.getFirstEditValue(cmbKasMasuk);
        ...
        public void setGrid(MySqlCommand command) {
            ...
            String strngKasMasuk = cmbKasMasuk.EditValue.ToString();
            String strngCatatan = txtCatatan.Text;


            String query = @"SELECT A.kode AS Nomor, A.tanggal AS Tanggal, B.kode AS 'Kode Akun Kas Masuk', B.nama AS 'Nama Akun Kas Masuk', A.nourut AS 'No Urut', A.total AS Total,A.catatan AS Catatan
                            FROM kasmasuk A
                            INNER JOIN akun B ON A.akun = B.kode
                            WHERE toDate(A.tanggal) BETWEEN toDate(@tanggalawal) AND toDate(@tanggalakhir) AND                                   
                                  A.kode LIKE @kode AND A.catatan LIKE @catatan AND A.akun LIKE @akun
                            ORDER BY A.kode";
            ...
            parameters.Add("akun", strngKasMasuk);
            parameters.Add("catatan", "%" + strngCatatan + "%");
        ...
        private void btnUbah_Click(object sender, EventArgs e) {
            FrmKasMasukAdd form = new FrmKasMasukAdd(false);
            MySqlConnection con = new MySqlConnection(OswConfig.KONEKSI);
            MySqlCommand command = con.CreateCommand();
            MySqlTransaction trans;
        ...
        private void btnTambah_Click(object sender, EventArgs e) {
            FrmKasMasukAdd form = new FrmKasMasukAdd(true);
            this.AddOwnedForm(form);
            form.ShowDialog();
        }
        ...
        private void btnHapus_Click(object sender, EventArgs e) {
            ...
            try {
                ...
                if(OswPesan.pesanKonfirmasiHapus() == DialogResult.Yes) {
                    ...
                    // tulis log
                    OswLog.setTransaksi(command, "Hapus " + dokumen, dKasMasuk.ToString());
