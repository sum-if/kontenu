...
namespace OmahSoftware.Sistem {
    public partial class SisLogTransaksi : DevExpress.XtraEditors.XtraForm {
        private String id = "LOGTRANSAKSI";
        private String dokumen = "Log Transaksi";

        public SisLogTransaksi() {
            InitializeComponent();
        }
        ...
        public void setGrid(MySqlCommand command) {
            ...
            String query = @"SELECT A.user AS 'Kode User', B.nama AS 'Nama', C.nama AS Usergroup, A.hostname AS Hostname, A.ip AS IP, A.dokumen AS Dokumen, 
                                    A.keterangan AS Keterangan, CAST(A.waktu AS CHAR) AS Waktu
                            FROM oswlogtransaksi A
                            ...
                            WHERE A.user LIKE @user AND A.hostname LIKE @hostname AND A.ip LIKE @ip AND DATE(A.waktu) BETWEEN toDate(@tanggalawal) AND toDate(@tanggalakhir) AND
                                  A.dokumen LIKE @dokumen AND A.keterangan LIKE @keterangan
            ...
            parameters.Add("keterangan", "%" + strngKeterangan + "%");
