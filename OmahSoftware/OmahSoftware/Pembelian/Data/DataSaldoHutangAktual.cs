...
namespace OmahSoftware.Pembelian {
    class DataSaldoHutangAktual {
        ...
        public DataSaldoHutangAktual(MySqlCommand command, String supplier) {
        ...
        public void tambah() {
            ...
            if(this.isExist) {
                ...
                double jumlahLama = double.Parse(dSaldoHutangAktual.jumlah);
                double jumlahBaru = jumlahLama + double.Parse(this.jumlah);

                if(jumlahBaru == 0) {
                    // habis hutangnya, maka harus di hapus dari saldo
                    this.hapus();
                } else if(jumlahBaru < 0) {
                    DataSupplier dSupplier = new DataSupplier(command, this.supplier);
                    throw new Exception("Saldo Hutang Aktual [" + this.supplier + "," + dSupplier.nama + "] < 0");
                } else {
                    this.version += 1;

                    // masih sisa
                    String query = @"UPDATE saldohutangaktual
                                     SET jumlah = @jumlah,
                                         version = @version,
                ...
                if(double.Parse(this.jumlah) > 0) {
                    ...
                    String query = @"INSERT INTO saldohutangaktual(supplier,jumlah,version,create_user) 
        ...
        private void hapus() {
            ...
            String query = @"DELETE FROM saldohutangaktual
                             WHERE supplier = @supplier";
        ...
        private void valNotExist() {
            if(this.isExist) {
                ...
                DataSupplier dSupplier = new DataSupplier(command, this.supplier);
                throw new Exception("Data Saldo Hutang Aktual [" + this.supplier + "," + dSupplier.nama + "] sudah ada");
            }
        }
        ...
        private void valExist() {
            if(!this.isExist) {
                ...
                DataSupplier dSupplier = new DataSupplier(command, this.supplier);
                throw new Exception("Data Saldo Hutang Aktual [" + this.supplier + "," + dSupplier.nama + "] tidak ada");
            }
        }

        private void valVersion() {
            DataSaldoHutangAktual dSaldoHutangAktual = new DataSaldoHutangAktual(command, this.supplier);
            if(this.version != dSaldoHutangAktual.version) {
                throw new Exception("Data Saldo Hutang Aktual yang dimiliki bukan yang terbaru, silahkan tutup dan lakukan proses ulang");
            }
        }
