...
namespace OmahSoftware.Pembelian {
    public partial class FrmPembayaranHutangEkspedisi : DevExpress.XtraEditors.XtraForm {
        private String id = "PEMBAYARANHUTANGEKSPEDISI";
        private String dokumen = "PEMBAYARANHUTANGEKSPEDISI";

        public FrmPembayaranHutangEkspedisi() {
            InitializeComponent();
        }

        private void FrmPembayaranHutangEkspedisi_Load(object sender, EventArgs e) {
            ...
            try {
                ...
                cmbEkspedisi = ComboQueryUmum.getEkspedisi(cmbEkspedisi, command, true);
                cmbEkspedisi.EditValue = OswCombo.getFirstEditValue(cmbEkspedisi);

                cmbAkunBayar = ComboQueryUmum.getAkun(cmbAkunBayar, command, "akun_pembayaran_hutang_ekspedisi", true);
        ...
        public void setGrid(MySqlCommand command) {
            ...
            String query = @"SELECT A.kode AS Nomor, A.tanggal AS Tanggal, B.nama AS Ekspedisi, C.nama AS 'Pembayaran Via', A.nourut AS 'No Urut', A.total AS 'Total Bayar', A.catatan AS Catatan
                            FROM pembayaranhutangekspedisi A
                            INNER JOIN ekspedisi B ON A.ekspedisi = B.kode
                            ...
                            WHERE toDate(A.tanggal) BETWEEN toDate(@tanggalawal) AND toDate(@tanggalakhir) AND                                   
                                  A.kode LIKE @kode AND A.ekspedisi LIKE @ekspedisi AND A.akunpembayaran LIKE @akunpembayaran
                            ORDER BY A.kode";
            ...
            parameters.Add("ekspedisi", strngEkspedisi);
        ...
        private void btnUbah_Click(object sender, EventArgs e) {
            FrmPembayaranHutangEkspedisiAdd form = new FrmPembayaranHutangEkspedisiAdd(false);
            MySqlConnection con = new MySqlConnection(OswConfig.KONEKSI);
            MySqlCommand command = con.CreateCommand();
            MySqlTransaction trans;
        ...
        private void btnTambah_Click(object sender, EventArgs e) {
            FrmPembayaranHutangEkspedisiAdd form = new FrmPembayaranHutangEkspedisiAdd(true);
            this.AddOwnedForm(form);
            form.ShowDialog();
        }
        ...
        private void btnHapus_Click(object sender, EventArgs e) {
            ...
            try {
                ...
                if(OswPesan.pesanKonfirmasiHapus() == DialogResult.Yes) {
                    ...
                    dPembayaranHutangEkspedisi.hapus();

                    // tulis log
                    OswLog.setTransaksi(command, "Hapus " + dokumen, dPembayaranHutangEkspedisi.ToString());
