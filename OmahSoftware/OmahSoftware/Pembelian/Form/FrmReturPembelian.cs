...
namespace OmahSoftware.Pembelian {
    public partial class FrmReturPembelian : DevExpress.XtraEditors.XtraForm {
        ...
        private void FrmReturPembelian_Load(object sender, EventArgs e) {
        ...
        public void setGrid(MySqlCommand command) {
            ...
            String query = @"SELECT A.kode AS 'No. Retur', A.tanggal AS 'Tgl Retur', B.nama AS Supplier, D.nama AS Kota, A.fakturpembelian AS 'No. Faktur Pembelian', C.faktursupplier AS 'No Faktur Supplier', A.grandtotal AS 'Total Retur', A.catatan AS Catatan
                            FROM returpembelian A
                            INNER JOIN supplier B ON A.supplier = B.kode
                            INNER JOIN fakturpembelian C ON A.fakturpembelian = C.kode
                            INNER JOIN kota D ON B.kota = D.kode
                            WHERE toDate(A.tanggal) BETWEEN toDate(@tanggalawal) AND toDate(@tanggalakhir) AND                                   
                                  A.kode LIKE @kode AND A.supplier LIKE @supplier
                            ORDER BY A.kode";
        ...
        private void btnUbah_Click(object sender, EventArgs e) {
            FrmReturPembelianAdd form = new FrmReturPembelianAdd(false);
            MySqlConnection con = new MySqlConnection(OswConfig.KONEKSI);
            MySqlCommand command = con.CreateCommand();
            MySqlTransaction trans;
            ...
            try {
                ...
                String strngKode = gridView.GetRowCellValue(gridView.FocusedRowHandle, "No. Retur").ToString();
        ...
        private void btnTambah_Click(object sender, EventArgs e) {
            FrmReturPembelianAdd form = new FrmReturPembelianAdd(true);
            this.AddOwnedForm(form);
            form.ShowDialog();
        }
        ...
        private void btnHapus_Click(object sender, EventArgs e) {
            ...
            try {
                ...
                if(OswPesan.pesanKonfirmasiHapus() == DialogResult.Yes) {
                    ...
                    dReturPembelian.hapus();

                    // tulis log
                    OswLog.setTransaksi(command, "Hapus " + dokumen, dReturPembelian.ToString());
