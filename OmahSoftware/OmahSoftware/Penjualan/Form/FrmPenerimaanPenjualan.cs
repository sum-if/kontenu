...
namespace OmahSoftware.Penjualan {
    public partial class FrmPenerimaanPenjualan : DevExpress.XtraEditors.XtraForm {
        private String id = "PENERIMAANPENJUALAN";
        private String dokumen = "PENERIMAANPENJUALAN";

        public FrmPenerimaanPenjualan() {
            InitializeComponent();
        }

        private void FrmPenerimaanPenjualan_Load(object sender, EventArgs e) {
        ...
        public void setGrid(MySqlCommand command) {
            ...
            String query = @"SELECT A.kode AS Nomor, A.tanggal AS Tanggal, B.nama AS Customer, D.nama AS Kota, A.jenis AS Jenis , C.nama AS 'Akun Deposit', A.nourut AS 'No Urut', A.nocek AS 'No. Cheque', A.catatan AS Catatan, A.total AS Total
                            FROM penerimaanpenjualan A
                            INNER JOIN customer B ON A.customer = B.kode
                            INNER JOIN akun C ON A.akunpenerimaan = C.kode
                            INNER JOIN kota D ON B.kota = D.kode
                            WHERE toDate(A.tanggal) BETWEEN toDate(@tanggalawal) AND toDate(@tanggalakhir) AND 
                                  A.kode LIKE @kode AND A.customer LIKE @customer AND A.jenis LIKE @jenis 
                            ORDER BY A.kode";
        ...
        private void btnUbah_Click(object sender, EventArgs e) {
            FrmPenerimaanPenjualanAdd form = new FrmPenerimaanPenjualanAdd(false);
            MySqlConnection con = new MySqlConnection(OswConfig.KONEKSI);
            MySqlCommand command = con.CreateCommand();
            MySqlTransaction trans;
        ...
        private void btnTambah_Click(object sender, EventArgs e) {
            FrmPenerimaanPenjualanAdd form = new FrmPenerimaanPenjualanAdd(true);
            this.AddOwnedForm(form);
            form.ShowDialog();
        }
        ...
        private void btnHapus_Click(object sender, EventArgs e) {
            ...
            try {
                ...
                if(OswPesan.pesanKonfirmasiHapus() == DialogResult.Yes) {
                    ...
                    dPenerimaanPenjualan.hapus();

                    // tulis log
                    OswLog.setTransaksi(command, "Hapus " + dokumen, dPenerimaanPenjualan.ToString());
