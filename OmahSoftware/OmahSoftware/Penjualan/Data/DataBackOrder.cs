...
namespace OmahSoftware.Penjualan {
    class DataBackOrder {
        ...
        public DataBackOrder(MySqlCommand command, String kode) {
        ...
        public void tambah() {
            ...
            String query = @"INSERT INTO backorder(kode, tanggal, customer, sales,jenisppn,total,diskon,diskonnilai,totaldiskon,totaldpp,totalppn,grandtotal,catatan, status, version,create_user) 
        ...
        public void hapus() {
            ...
            // hapus header
            String query = @"DELETE FROM backorder
                             WHERE kode = @kode";
        ...
        public void hapusDetail() {
            ...
            parameters.Add("backorder", this.kode);
            ...
            foreach(DataRow row in dataTable.Rows) {
                DataBackOrderDetail dBackOrderDetail = new DataBackOrderDetail(command, this.kode, row["no"].ToString());
                dBackOrderDetail.hapus();
            }
        }
        ...
        private void valVersion() {
            DataBackOrder dBackOrder = new DataBackOrder(command, this.kode);
            if(this.version != dBackOrder.version) {
        ...
        public void valFakturPenjualan() {
            // cek sudah ada faktur penjualan atau belum di barang
            String query = @"SELECT COUNT(*)
                             FROM backorderdetail A
                             WHERE A.backorder = @kode AND A.jumlahfaktur > 0";
