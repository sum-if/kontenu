...
namespace OmahSoftware.Akuntansi {
    public partial class FrmLaporanPriceListUpdate : DevExpress.XtraEditors.XtraForm {
        private String id = "LAPORANPRICELIST";
        private String dokumen = "LAPORANPRICELIST";

        public FrmLaporanPriceListUpdate() {
            InitializeComponent();
        }

        private void FrmLaporanPriceListUpdate_Load(object sender, EventArgs e) {
        ...
        public void setGrid(MySqlCommand command, bool awal = false) {
            ...
            String query = @"SELECT B.kode AS 'Kode Kategori', B.nama AS Kategori, A.kode AS 'Kode Barang', A.nama AS Barang, A.nopart AS 'No Part', C.nama AS Satuan, 
                            ...
                            WHERE A.status = 'Aktif' AND A.barangkategori LIKE @kategori AND A.kode LIKE @kodebarang AND A.nama LIKE @namabarang AND
                                  toDate(A.tanggalhargaupdate) BETWEEN toDate(@tanggalAwal) AND toDate(@tanggalAkhir)
        ...
        private void btnCetak_Click(object sender, EventArgs e) {
            ...
            filter.Add("Tanggal Perubahan", strngTanggalAwal + " - " + strngTanggalAkhir);
            ...
            Tools.cetakLaporan(gridControl, dokumen, filter, lebar, false, 100,
                               new System.Drawing.Printing.Margins(25, 25, 25, 25));
        }

        private void btnCetakPriceListUpdate_Click(object sender, EventArgs e) {
            SplashScreenManager.ShowForm(typeof(SplashUtama));
            MySqlConnection con = new MySqlConnection(OswConfig.KONEKSI);
            MySqlCommand command = con.CreateCommand();
            MySqlTransaction trans;
            ...
            try {
                ...
                RptLaporanPriceListUpdate report = new RptLaporanPriceListUpdate();
